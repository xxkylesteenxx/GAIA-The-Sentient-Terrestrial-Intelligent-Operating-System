...
try:
    ...
    print("Warning: Could not import GAIA modules. Some features may be unavailable.")
    ZScoreCalculator = None
    AvatarPersonality = None
...
class GAIASession:
    ...
    def __init__(self):
        ...
        self.z_calculator = ZScoreCalculator() if ZScoreCalculator else None
        self.avatar = None
        self.logos = None
        
        if self.config and AvatarPersonality:
...
def status():
    ...
    if session.z_calculator:
        ...
        console.print(f"\n[yellow]Z Score calculation unavailable[/yellow]")
...
def speak(message):
    ...
    if session.logos:
        ...
        console.print("[yellow]Logos interpreter not available[/yellow]\n")
